...
jittered. The active runtime frames are now canonical; the redundant archived
copies were removed. Their intentional pose-height and foot-placement variation
is preserved rather than forced onto a uniform baseline.
...
The pogo uses its own 11-frame `pogo_attack_v2.png` atlas rather than borrowing
...
The reviewed ledge-climb runtime sheet is canonical. Its redundant individual
frame copies were removed from the runtime asset tree.
