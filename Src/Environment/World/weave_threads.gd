...
@export var endpoint_light_energy := 1.05
@export var endpoint_light_scale := 9.5
@export var endpoint_blend_light_energy := 0.62
@export var endpoint_blend_light_scale := 12.0
...
var _endpoint_lights: Array[PointLight2D] = []
var _endpoint_blend_light: PointLight2D
...
func _build_beams() -> void:
	for spec: Dictionary in _thread_specs():
		...
		var endpoint_light := PointLight2D.new()
		endpoint_light.name = "%sEndpointLight" % spec["name"]
		endpoint_light.z_index = -1
		endpoint_light.texture = THREAD_LIGHT_TEXTURE
		endpoint_light.texture_scale = endpoint_light_scale
		endpoint_light.color = spec["light_color"] as Color
		endpoint_light.energy = endpoint_light_energy
		endpoint_light.range_z_min = -4096
		endpoint_light.range_z_max = 4096
		add_child(endpoint_light)
		_endpoint_lights.append(endpoint_light)

	_endpoint_blend_light = PointLight2D.new()
	_endpoint_blend_light.name = "ThreadEndpointBlendLight"
	_endpoint_blend_light.z_index = -1
	_endpoint_blend_light.texture = THREAD_LIGHT_TEXTURE
	_endpoint_blend_light.texture_scale = endpoint_blend_light_scale
	_endpoint_blend_light.color = Color(1.0, 0.9, 0.68, 1.0)
	_endpoint_blend_light.energy = endpoint_blend_light_energy
	_endpoint_blend_light.range_z_min = -4096
	_endpoint_blend_light.range_z_max = 4096
	add_child(_endpoint_blend_light)
...
func _update_beams() -> void:
	...
	for index in range(specs.size()):
		...
		_endpoint_lights[index].position = points[0]
	...
	if _endpoint_blend_light:
		_endpoint_blend_light.position = loom_origin
