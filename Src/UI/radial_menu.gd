...
# Slot Controls - using your exact node names
...
func _ready() -> void:
	...
	print("=== Radial Menu Slot Load Debug ===")
	...
	# Connect slots - no more ThreadColor needed
	_connect_slot(monarch_gloves_slot, 0)
	_connect_slot(monarch_boots_slot, 3)
	_connect_slot(monarch_chest_slot, 6)
	_connect_slot(hermit_gloves_slot, 1)
	_connect_slot(hermit_boots_slot, 4)
	_connect_slot(hermit_chest_slot, 7)
	_connect_slot(sage_gloves_slot, 2)
	_connect_slot(sage_boots_slot, 5)
	_connect_slot(sage_chest_slot, 8)

func _connect_slot(slot: Control, slot_idx: int) -> void:
	if slot == null:
		print("Warning: slot is null for index ", slot_idx)
		return
...
func _fade_in_blur() -> void:
	if blur_layer: blur_layer.visible = true
	if blur_rect:
